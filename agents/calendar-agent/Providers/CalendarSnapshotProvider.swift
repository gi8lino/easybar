...
final class CalendarSnapshotProvider {
    ...
    func start(onChange: @escaping () -> Void) {
        ...
        AgentLogger.info("calendar agent authorization status before start=\(authState.describe(status))")
    ...
    func snapshot(for query: CalendarAgentQuery) -> CalendarAgentSnapshot {
        ...
        guard let endDate = calendar.date(byAdding: .day, value: max(1, query.days), to: startOfToday) else {
            return CalendarAgentSnapshot(
                accessGranted: true,
                permissionState: permissionState,
                generatedAt: now,
    ...
    private func requestAccessIfNeeded() {
        ...
        case .notDetermined:
            ...
            eventStore.requestFullAccessToEvents { [weak self] granted, error in
                ...
                if let error {
                    AgentLogger.error("calendar agent access request failed status=\(self.authState.describe(newStatus)) error=\(error)")
                    return
                }

                AgentLogger.info("calendar agent access request completed granted=\(granted) status=\(self.authState.describe(newStatus))")
