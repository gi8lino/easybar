...
Boolean-like values accepted by some properties. EasyBar accepts both native Lua booleans and the string forms `"on"` and `"off"`.
...
| Value                                       |
| ------------------------------------------- |
| `string\|number\|boolean\|EasyBarIconProps` |
...
| Value                                        |
| -------------------------------------------- |
| `string\|number\|boolean\|EasyBarLabelProps` |

## `EasyBarPosition`

Placement string for a node. At the root level this is usually `left`, `center`, or `right`; popup children use `popup.<parent-id>`.
